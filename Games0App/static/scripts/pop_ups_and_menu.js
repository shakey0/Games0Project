document.addEventListener('DOMContentLoaded', function() {
    ...
    const openBoxButtons = document.querySelectorAll('[data-legal-button-target], [data-about-button-target], [data-login-button-target], [data-forgotten-password-button-target], [data-register-button-target], [data-amend-score-box-target], [data-quit-game-target]');
    ...
    openBoxButtons.forEach(button => {
        button.addEventListener('click', () => {
            const boxSelector = button.dataset.legalButtonTarget || button.dataset.aboutButtonTarget || button.dataset.loginButtonTarget || button.dataset.forgottenPasswordButtonTarget || button.dataset.registerButtonTarget || button.dataset.amendScoreBoxTarget || button.dataset.quitGameTarget;
    ...
    cancelBoxButtons.forEach(button => {
        button.addEventListener('click', () => {
            const box = button.closest('.confirm-legal-box, .legal-box, .about-box, .login-box, .forgotten-password-box, .register-box, .amend-score-box, .quit-game-box');
            closeBox(box);
        });
    });
    ...
    overlay.addEventListener('click', () => {
        const boxes = document.querySelectorAll('.legal-box.active, .about-box.active, .quit-game-box.active');
        boxes.forEach(box => {
            closeBox(box);
        });
    });
